...
const args = yargs
  ...
  .example(
    "$0 --in ./contracts -o ./docs",
    "Render `.sol` files in `./contracts` into `.docs`"
  )
  .options({
    in: {
    ...
    json: {
      alias: "j",
      desc:
        "Output the parsed information to a json file instead of rendering.",
      type: "string",
    ...
    theme: {
      alias: "t",
      desc:
        "Specifies a npm module that exports a default `render(filepath,contractName,contactInfo,options): {content,extension}` function",
      type: "string",
    ...
    "repo-url": {
      alias: "r",
      desc:
        "Specifies remote repository url. Uses `repository` field in `package.json` if available and not specified.",
      type: "string",
