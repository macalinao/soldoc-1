    ...
    const result = solc.compile({ sources: data }, 1, file => {
      ...
      return {
        contents: fs.readFileSync(
          fs.existsSync(node_path) ? node_path : file,
          "utf-8"
        )
      };
    });
...
const clean = data => {
  ...
  const signature = (obj, names) =>
    `${obj.name}(${Object.keys(obj.params)
      .map(
        k =>
          names
            ? `${obj.params[k].type} ${obj.params[k].name}`
            : obj.params[k].type
      )
      .join(",")})`;
  const normalize = obj => ({
    ...obj,
    params:
      obj.params || obj.inputs
        ? (obj.params || obj.inputs).reduce(
            (acc, p, i) => ({
              ...acc,
              [p.name || `param_${i}`]: { ...p, name: undefined }
            }),
            {}
          )
        : undefined,
    outputs: obj.outputs
      ? obj.outputs.reduce(
          (acc, p, i) => ({
            ...acc,
            [p.name || `output_${i}`]: { ...p, name: undefined }
          }),
          {}
        )
      : undefined,
  ...
  const mapObj = (obj, f) =>
    Object.keys(obj || {}).reduce((acc, k) => ({ ...acc, [k]: f(obj[k]) }), {});
  ...
  const events = abi
    ...
    .map(normalize)
    .reduce((acc, e) => ({ ...acc, [signature(e)]: e }), {});
  ...
  const methods = abi
    ...
    .sort((a, b) => a.name > b.name)
    .map(normalize)
    .reduce((acc, f) => ({ ...acc, [signature(f)]: f }), {});
  ...
  const docs = {
    ...
    methods: {
      ...mapObj(merged.methods, doc => ({
        ...doc,
        params: mapObj(doc.params, p => ({ details: p }))
      })),
      [""]: undefined
    }
  };
  ...
  const gas = {
    ...
    fallback: data.gasEstimates.external[""]
      ? { executionCost: data.gasEstimates.external[""] }
      : null,
    methods: {
      ...mapObj(data.gasEstimates.external, x => ({ executionCost: x })),
      [""]: undefined
    }
  };
...
const soldoc = options => {
  ...
  const log = (tag, ...objs) => {
    ...
    if (opts.log)
      fse.appendFileSync(
        opts.log,
        `${new Date().toISOString()} soldoc ${tag}: ${objs}\n`
      );

    if (!opts.quiet)
      shelljs.echo(
        chalk`{gray ${new Date().toISOString()}} {blue soldoc} {${color[tag] ||
          "gray"} ${tag}}: ${objs}`
      );
  };
    ...
    if (!opts.repoUrl) {
      ...
      if (package.repository)
        if (typeof package.repository === "object")
          ...
          (validUrl.isHttpUri(package.repository) ||
            validUrl.isHttpsUri(package.repository))
        )
  ...
  return new Promise((resolve, reject) => {
    ...
    .then(files =>
      Promise.all(
        files.map(f =>
          fse.readFile(f, "utf8").then(content => ({ [f]: content }))
        )
      )
    )
    ...
    .then(extracted => {
      if (opts.json) {
        ...
        return fse.writeFile(
          opts.json,
          JSON.stringify(extracted, undefined, 2),
          "utf8"
        );
      } else {
        return Promise.all(
          Object.keys(extracted).map(f =>
            Promise.all(
              Object.keys(extracted[f]).map(contract => {
                ...
                const where = path.resolve(
                  opts.out,
                  path.relative(opts.in, path.dirname(f)),
                  `${contract}${result.extension}`
                );
